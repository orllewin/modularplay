...
function LowpassMod:redrawBackground()
...
function LowpassMod:turn(x, y, change)
	...
	if encoderVector == self.freqEncoderVector then
		...
		self.component:setFrequency(map(self.freqEncoderValue, 0.0, 1.0, 0.0, 1000.0))
...
function LowpassMod:invalidate()
	if self.redrawPending then
		self:redrawBackground()
	end
end

function LowpassMod:findClosestEncoderVector(x, y)
...
function LowpassMod:tryConnectGhostIn(x, y, ghostCable, sourceSubtype)
...
function LowpassMod:getModId()
	return self.modId
end

function LowpassMod:handleModClickPedal(tX, tY)
	self.upPending = true
	self:moveBy(0, 2)
...
function LowpassMod:handleModClickUp(tX, tY, listener)
...
function LowpassMod:setChannel(channel, onNext)
	if channel == nil then
...
function LowpassMod:unplug(cableId)
	self.component:unplug(cableId)
end

function LowpassMod:repositionBy(x, y)
	self:moveBy(x, y)
end

function LowpassMod:moveFinish()
