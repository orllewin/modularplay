...
import 'Modules/Drums/OR909/or909_component'

class('OR909Mod').extends(playdate.graphics.sprite)
...
local modType = "OR909Mod"
local modSubtype = "audio_gen"

local drumSelector = gfx.image.new("Images/drum_selector_909")
...
function OR909Mod:init(xx, yy, modId, onInit)
	OR909Mod.super.init(self)
	...
	self.component = OR909Component(function(channel)
	...
	gfx.drawText("Orllewin", 42, 22)
	local bigFont = assets:bigFont()
	bigFont:drawText("OR-909", 40, 32)
...
function OR909Mod:unplug(cableId)
...
function OR909Mod:repositionBy(x, y)
	self:moveBy(x, y)
	self.steps:repositionBy(x, y)
end

function OR909Mod:moveFinish()	
...
function OR909Mod:evaporate(onDetachConnected)
	--first detach cables
...
function OR909Mod:redrawBackground()
	gEncoder:hide()
...
function OR909Mod:turn(x, y, change)
...
function OR909Mod:invalidate()
	if self.redrawPending then
		self:redrawBackground()
	end
end

function OR909Mod:findClosestEncoderVector(x, y)
...
function OR909Mod:type()
	return modType
end

function OR909Mod:getModId()
	return self.modId
end

function OR909Mod:handleModClick(tX, tY, listener)
...
function OR909Mod:collision(x, y)
...
function OR909Mod.ghostModule()
	return buildGhostModule(moduleWidth, moduleHeight)
end

function OR909Mod:tryConnectGhostIn(x, y, ghostCable)
...
function OR909Mod:tryConnectGhostOut(x, y, ghostCable)
	if self.component:outConnected() then
		return false
	else
...
function OR909Mod:fromState(modState)
