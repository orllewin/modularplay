...
import 'Modules/Clocks/Bifurcate4/bifurcate4_component'
...
function Bifurcate4Mod:init(xx, yy, modId)
	...
	self.component = Bifurcate4Component()
end

function Bifurcate4Mod:onRotate()
	local backgroundImage = generateModBackgroundWithShadow(moduleWidth, moduleHeight)
...
function Bifurcate4Mod:getModId()
	return self.modId
end
...
function Bifurcate4Mod:unplug(cableId)
	self.component:unplug(cableId)
end

function Bifurcate4Mod:repositionBy(x, y)
	self:moveBy(x, y)
end

function Bifurcate4Mod:moveFinish()	
	if self.rotation == rotationRight then
		self.socketInVector = Vector(self.x - 25, 	self.y)
		self.socketOutAVector = Vector(self.x - 15, self.y + 25)
		self.socketOutBVector = Vector(self.x + 15, self.y + 25)
